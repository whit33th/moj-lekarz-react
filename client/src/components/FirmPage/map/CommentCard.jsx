...
import styles from "./Card.module.css"; // Assume you have your styles here
...
const Card = ({ review }) => {
  ...
  return (
    <div className={styles.card}>
        ...
        <p className={`${styles.cardText} ${showMore ? styles.showMoreText : ''}`}>{review.text}</p>
        {review.text.length > 200 && <p style={{
          color: '#3E36B0', textAlign: 'right', cursor: 'pointer',padding: '10px'
        }} onClick={showMoreText} className={styles.showMore}>{showMoreStatus?'Rozwiń' : 'Zwiń'}</p>}
        </div>
      </div>
