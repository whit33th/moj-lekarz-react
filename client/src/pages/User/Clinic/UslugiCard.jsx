  ...
  return (
    <div className={styles.uslugiBlock}>
      ...
      <div className={styles.column}>
        {uslugiAndPrice
          .slice(0, Math.ceil(uslugiAndPrice.length / 2))
          .map((item, index) => (
            <div
              className={`${styles.uslugiItem} ${
                uslugiItemState === index ? styles.active : ""
              }`}
              key={index}
            ...
            >
              <div className={styles.titleBlock}>
                <div>
                  {item.name} <span>{item.list.length}</span>
                </div>
                <div
                  className={`${styles.uslugiItemArrow} ${
                    uslugiItemState === index ? styles.activeArrow : ""
                  }`}
                ></div>
              </div>
      ...
      <div className={styles.column}>
        {uslugiAndPrice
          .slice(Math.ceil(uslugiAndPrice.length / 2))
          .map((item, index) => (
            <div
              className={`${styles.uslugiItem} ${
              ...
              onClick={() =>
                toggleItem(index + Math.ceil(uslugiAndPrice.length / 2))
              }
            >
              <div className={styles.titleBlock}>
                <div>
              ...
              {uslugiItemState ===
                index + Math.ceil(uslugiAndPrice.length / 2) && (
                <div className={styles.contentBlock}>
                  {item.list.map((element) => (
                    <div key={element.name}>
                      <div className={styles.uslugiTypeItem} key={element.name}>
